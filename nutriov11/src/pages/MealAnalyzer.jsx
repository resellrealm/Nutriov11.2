import React, { useState } from 'react';
  ...
  const getScanCount = () => {
    const key = 'scanCount:' + getTodayKey();
...
const MealAnalyzer = () => {
  ...
  const analyzeMeal = () => {
    /*SCAN_GUARD*/
    if (planTier === 'basic' && getScanCount() >= 1) {
      alert('Daily scan limit reached for Basic plan. Upgrade to Premium for unlimited image scans. Barcode scans and manual entry remain available.');
      return;
    }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-card p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <input type="text" inputMode="numeric" pattern="[0-9]*" placeholder="Enter or scan barcode (EAN/UPC)"
                 className="w-full border rounded-lg p-2 mb-2" />
          <p className="text-sm text-gray-500">Tip: On mobile, focus the field and use your camera’s scan button to fill the barcode. Unlimited for all plans.</p>
        </div>
      </div>
    </div>
  );
