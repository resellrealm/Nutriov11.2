      ...
      <AnimatePresence>
        {isOpen && (
          <motion.aside
            ...
            className="fixed top-0 left-0 h-full w-72 bg-white dark:bg-gray-800 shadow-xl z-50 overflow-y-auto pt-[max(16px,env(safe-area-inset-top))]"
          >
            {/* Sidebar Header */}
                ...
                {navItems.map((item) => {
                  ...
                  return (
                    <li key={item.path}>
                      ...
                      >
                        <Icon size={20} />
                        <span>{item.label}{planTier==='basic' && (item.path==='/goals' || item.path==='/meal-planner') && <Lock className='w-4 h-4 ml-2' />}</span>
                      </NavLink>}
                    </li>
                  );
                })}
